report 50000 "StandardSalesInvoice"
{
    RDLCLayout = './SalesInvoiceCBR.rdl';
    WordLayout = './SalesInvoiceCBR.docx';
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                column(LineAmount_Line; CurrencySymbol + FormattedLineAmount)//AGT_DS
                {
                    AutoFormatExpression = GetCurrencyCode();
                    AutoFormatType = 1;
                }
                ...
                column(UnitPrice; CurrencySymbol + FormattedUnitPrice)//AGT_ds
                {
                    AutoFormatExpression = GetCurrencyCode();
                    AutoFormatType = 2;
                }
                ...
                column(TotalAmountlines; TotalAmountlines)
                {
                ...
                dataitem("Value Entry"; "Value Entry")
                {
                    ...
                    dataitem("Item Ledger Entry"; "Item Ledger Entry")
                    {
                        ...
                        column(Quantity; ABS("Item Ledger Entry".Quantity))
                        {
                        }
                ...
                begin
                    //CBR_SV 091421>>
                    if Type <> Type::" " then
                        ReportSkip := SkipSalesInvLines(Line);
                    ...
                    TotalAmountlines := Line."Unit Price";
                    GenLedSetup_G.reset;
                    Clear(CurrencySymbol);
                    If GenLedSetup_G.FindFirst() then
                        If FormattedQuantity <> '' then
                            CurrencySymbol := GenLedSetup_G."Local Currency Symbol";
            ...
            dataitem(Totals; "Integer")
            {
                ...
                column(TotalAmountIncludingVAT; GenLedSetup_G."Local Currency Symbol" + Format(TotalAmountInclVAT, 0, AutoFormat.ResolveAutoFormat("Auto Format"::AmountFormat, Header."Currency Code")))
                {
                }
            ...
            begin
                ...
                IF Rec_Payment_terms.Get(Header."Payment Terms Code") then;
    ...
    var
        ...
        TotalAmountlines: Decimal;
        RecordSkip: Boolean;
        QtyAvailable: Boolean;
        ReportSkip: Boolean;
        GenLedSetup_G: Record "General Ledger Setup";//AGT_DS
        CurrencySymbol: Text;
    ...
    local procedure SkipSalesInvLines(DataItemSalesInvLin: Record "Sales Invoice Line"): Boolean
    begin
        //Skiplines-YK09142021
        if DataItemSalesInvLin.Quantity > 0 then begin
            RecordSkip := false;
            QtyAvailable := true;
        end else begin
            QtyAvailable := false;
        ...
        if (RecordSkip) and not (QtyAvailable) then
            exit(true)
        else
            exit(false);
        //Skiplines-YK09142021
